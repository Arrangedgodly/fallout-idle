...
so 1 + 4 rungs = 5 postings = all five skills concurrent (town-hall Scope
Amendment 1). Prices are balance data: T20 owns the tuning; scripts never
hardcode them (`ContentLibrary.deputy_price_at(rung)` is the read path).

| Field | Type | Range | Req | Notes |
|---|---|---|---|---|
| `id` | string | snake_case | y | descriptive rung id (`second_deputy`...) |
| `price` | int | 1–1,000,000 | y | Crowns the DEPUTIZE RESIDENT button charges at this rung |
