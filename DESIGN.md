...
- **Plate wall** (left ~third, T35 — the T30 card grid reverted to the list): a single column of eight full-width department plates (2 px list separation, the pre-T30 density), vertical scroll reserved for 200% font scale. Each plate: the full stencil name with its designation digit (`SCAVENGING · 1` — the digit is the D-0n serial number and the hotkey), a mono readout, and — the T30 additions the revert RETAINS — a 7 px XP micro gauge (`MicroGauge`: hairline recessed track, 3 px amber enamel fill — skills only, riding the readout row's right end) and the filled deputy badge when the department holds a posting (skills only, at the name row's right shoulder). Readouts: skill plates post `CLR NN · into/step` (at max grade, `CLR NN · MAX`); the Depot posts its wallet; the Manifest its holdings count (`N LINES`); Personnel the posting count (`N/M POSTED`). Reading order = department order (top to bottom); the tab chain and hotkeys 1-8 follow it; arrow keys walk the column. (Run-5 history: the T30 2-column compact card grid with short forms — SCAV/FORAGE/FORGE/COOK/PATROL/DEPOT — was superseded by the user's run-6 correction, which moves the condensed-card treatment INSIDE the dockets; see the Run 6 addendum.)
...
- **Docket internals** (skill dockets): hidden-at-rest energized status plate → vent housing with the clearance gauge (a 12 px enamel line since T30 — the honest digits live in the mono readout below it) + mono readout → micro section header → the compact item-card grid (T35: a 2-up `GridContainer`, 8 px separations, collapsing to one column where the width does not fit — see the Run 6 addendum) → vent-housed stamped log.
...
Progress with the recessed-track + amber-fill material: track = machined inset (steel-deep, 2/1 px border, 3 px inner margin), fill = amber at 1 px radius; 20 px minimum height (12 px on the XP meters since T30 — see the Run 5 addendum); no percentage text — the exact numbers sit in the mono readout line beneath ("CLEARANCE 02 · 20/65 XP TO NEXT"). Gauges also back both combat HP reads (RESIDENT / fauna) and the settings font-scale slider (recessed track, amber grabber area, enamel grabber disc; amber when lit). The card wall's `MicroGauge` is the same material at hairline margins — a 3 px amber line in a 7 px instrument.
...
Department cards (the wall) + TabBar where tabs exist: selected tab = bone plate with 2 px navy border and navy stencil; unselected = steel-deep with a 2 px steel-lo underline; hover tints steel-hi at 35%. The card's posted digit is a live accelerator: keys 1–8 (digit row and keypad) select their department from anywhere in the concourse — no Tab walk — with focus following to the destination card so the tab chain resumes into the new docket; a posted MAIL CALL owns the input while it is up, and modifier combos (Cmd/Ctrl/Alt + digit) stay with the OS. Scrollbars are thin steel (4 px radius grabber, steel-hi) going amber when highlighted, bone when pressed. Full keyboard reachability is part of the navigation contract: every visible enabled control is focusable, and the shell scrolls focus into view.
...
- Lifecycle (run-4 as-built): the FOLD control is posted from the very first stamp (step 0 — labeled, focusable, at the strip's right edge); at ≥ 5 stamps the form AUTO-folds to its slip once as a convenience (a manual OPEN sticks for the rest of the tutorial); the seventh stamp swaps the checklist for the record — **DULY ORIENTED · FORM O-1** + "ORIENTATION STIPEND — 150 CROWNS · THANK YOU FOR YOUR PROMPT COMPLIANCE." — holds a restrained 2.6 s beat, then settles into a permanent slip with an OPEN control (posted records never vanish). The stipend posts at the seventh stamp, i.e. immediately after the first deputize (T20 ruling, recorded in balance-notes §5.2).
- Low-text is a pinned budget: ≤ 40 words in every state (measured 33 fresh / 18 completed record / 4 slip). Placement (run-4 as-built, T29 — the de-blockade fix): the form is a DOCKED posted-paper strip at the top of the docket region — it takes layout space, the docket viewport resizes around it, and its checklist scrolls inside the strip against a docket-viewport floor; folded it collapses to the one-line slip (title + stamp boxes + OPEN). The run-2–3 form floated over the docket's top-right corner at z 4 with no fold below 5 stamps and ate every click inside its rect (measured: 313×466 px at 100 %, 593×466 at 200 % — ~28 % of the 720p shell — with interactive card regions of 5 of 8 dockets covered); the blockade-class audit now pins the law structurally: no non-modal input-present control may intersect any focusable control's rect at any state × scale × resolution, with the MAIL CALL modal the sole exemption.
...
## As-Built Addendum — Run 5: The Compact Card Wall (2026-09-18, T30)

The user's direction: "I don't like all the skills being massive bars, I like the idea of making them condensed cards more like Melvor's UI." The density rework keeps the Shelter Signage System vocabulary — enamel/steel/stencil/mono — and re-cuts the wall's geometry.

### The SkillCard wall (replaces the plate wall)

- **Grid:** a 2-column `GridContainer` (8 px separations) inside the same left scroll region (360 px floor; vertical scroll is reserved for 200 % font scale). Reading order = department order, row-major; the tab chain, hotkeys 1–8, and the grid arrows (within rows, then across) all follow it.
- **Card anatomy (`SkillCard` / `SkillCardEnergized` Button variations — the Button state pair at 10/6 margins):** 30 px department mark (icon-over-text) with the filled deputy badge at the row's right shoulder while the department holds a posting; the short stencil name + designation digit; the mono readout (`PlateSerialNavy` → `MonoValueEnergized` when live); a 7 px `MicroGauge` XP line on skill cards only.
- **Short-form register (stencil caps):** SCAV · FORAGE · FORGE · COOK · PATROL · DEPOT · MANIFEST · PERSONNEL — generic-English truncations of the cleared plate names (trademark sweep: zero hits; no new coined names). The digit after the name is the D-0n serial number and the hotkey; the full names stay on the docket headers and tooltips.
- **Role readouts replace grades on non-skill cards:** the Depot posts its wallet (`N CROWNS` — the crowns mark beside its own number, per the icon grammar), the Manifest its holdings (`N LINES`), Personnel the posting count (`N/M POSTED` — sized so the longest word fits a 200 % card; the T15 collapse guard holds). The Personnel card's mark is the deputy badge, the Depot's the crowns, the Manifest's a new `manifest_board` roster-sheet glyph (ASSETS row; the mark posts on the card only).
- **The energized state keeps every cue** (never color alone): the energized card variation (navy ground, amber border), the `>> ` prefix on the stencil name, the amber mono readout, and the 1.05 swell.
- **The O-1 cue re-posts:** the orient_arrow now sits at the destination card's LEFT shoulder, tip touching the card it names — the grid's column gap would strand a right-shoulder arrow between two cards.

### Gauge slimming

The docket XP meters (skill CLEARANCE GAUGE + the patrol's WASTELAND COMBAT CLEARANCE) slim 20 → 12 px (~40 %, the amendment's second clause). The condition/HP instruments keep their 20 px bodies — they are not XP meters. The gauges carry no text of their own at any height; the mono readouts keep every digit, so nothing clips at 200 %.

- **Icon set: 97 original SVGs** (the run-3 count of 96 + `manifest_board`; ASSETS.md rows match 97 ↔ 97).

## As-Built Addendum — Run 5: Tutorial Deep-Linking (2026-09-18, T33)

The user's words (Scope Amendment 3, complaint #4): "Tutorial was very unclear, especially for FILE A CROWN CLAIM that you had to scroll to the bottom of a very large window to sell a product. It took me over 5 minutes to figure out what the fuck the game wanted me to do." The fix is a per-step **reveal**: the game walks the resident to a control, not a department.

### The reveal (the O-1 form's wayfinding, made physical)

- **Trigger:** when a step becomes current (a stamp, or a boot into an incomplete tutorial) and when the resident presses the step's wayfinding row. Esc returns to the department the jump departed from (the keyboard way back; a manual plate/hotkey jump spends the memory).
- **Actions, in order:** select the target department card (the same bulkhead transition as a press) → post the right tab (FILE A CROWNS CLAIM lands the Depot on SELL; the patrol reveal posts the right zone) → scroll the docket so the target control is **fully in view** (the amendment's own words; re-asserted one layout pass later, because the O-1 strip's body budget re-flows the region) → mark the target with **one settle pulse**: a 0.5 s bounded warm lift toward the amber signal tint + a 2 % settle swell, back at rest — never a repeat, never a loop (motion discipline; the orient_arrow plate cue stays the non-color cue, the pulse is emphasis on arrival).
- **The target map (documented):** WORK A POSTED SHIFT → Scavenging's first workable tier card · EARN A CLEARANCE → the docket's clearance gauge block · FILE A CROWNS CLAIM → Depot, SELL tab, the first disposal row · PROCESS A PRODUCT → Junksmithing's first craftable recipe card · PROVISION THE PATROL → the Manifest EQUIP control (gear line pre-selected) when gear is on hand, else the cook leg (first craftable food recipe) — the engine stamps either leg; the reveal cues the reachable one · CLEAR A NUISANCE → the designated fauna card while fighting, else the first engageable card · DEPUTIZE A RESIDENT → the purchase row.
- **Honest prerequisites:** a step whose action is not yet reachable cues the action's SOURCE instead (the earliest-reachable gathering tier that yields the missing stock/input) and the step's stencil line gains its registered suffix — **WORK FOR INVENTORY FIRST** / **GATHER SUPPLIES FIRST** (naming-bible §10 T33 rows, Class A). The plate cue, the row tooltip, and the reveal all follow the honest fallback; the suffix withdraws the moment the action becomes reachable (copy never outlives its fact). The ≤ 6-word plate rule is untouched — the titles stay bible-verbatim; the suffix is the prerequisite mechanism.
- **Motion accounting:** the reveal's pulse is the deep link's only motion; it does not stack (a re-reveal restarts one bounded pulse, token-guarded, the T31 denial-flash discipline).
- **Acceptance:** tests/test_tutorial_reveal.gd — per-step reveal (department + tab + clipped-rect visibility + pulse-once), suffix logic, keyboard wayfinding + Esc return, and the structural walkthrough: a fresh save through all seven steps on the machinery alone in **23 UI actions** (budget 25; the user's five minutes is the baseline). Harnesses pinning pre-T33 shell geometry set `auto_reveal = false` (the seam; the wayfinding row always reveals — it is explicit intent).

## As-Built Addendum — Run 5 Close: Component Register + Acceptance Polish (2026-09-18, T34)

Scan-and-confirm pass over the whole run-5 surface (fresh captures of every department docket at 1280×720 and 200 % under `.impeccable/review/t34/`, all viewed): the shell composition is coherent at both scales — docket topology (enamel header plate → posted directive → live content → primary plate → footer serial) holds on all eight dockets, the compact grid rhythm is even, icon rows align, and the blockade audit is green across its states. What the captures DID show was fixed, and nothing else was touched. This addendum also registers the three run-5 components that shipped without a design-register entry (T31, T32) and confirms the two that have one (T30, T33).

### The refusal-strip component (T31, registered here)

- **Anatomy:** a `DangerPlate` `PanelContainer` pinned at child index 0 of its docket — in-flow, always the first content, never an overlay (the T29 docked-strip discipline). Row 1: the badge (or context) glyph + the `PlateTitleDanger` head + a DISMISS button. Row 2: the wrapped `MonoValue` serial (the reason). Row 3 (posting refusals only): the wrapped `BodyCopy` plan line + the REASSIGN button. On the red ground only the registered bone-ink pairs post (the DECEASED phase pairs; WCAG-AA).
- **Voice (naming-bible verbatim):** POSTING REFUSED / ALL POSTINGS ASSIGNED — CEASE ONE OR REASSIGN; CLEARANCE REQUIRED / CLEARANCE N REQUIRED — EARN IT IN THIS DEPARTMENT|ZONE; SUPPLIES MISSING / INSUFFICIENT SUPPLIES: N× ITEM; REQUEST REFUSED / the engine reason verbatim (kindless refusals — never silent). The plan restates the swap: REASSIGN — CEASE X, COMMENCE Y; the confirmation posts engine truth: POSTING REASSIGNED · CEASED X · Y NOW HOLDS THE POSTING.
- **Lifecycle (copy never outlives its fact):** truth-gated withdrawal in the refresh (a slot strip frees when a posting frees, a clearance strip when the grade is earned, a supplies strip when the stock suffices), plus the next successful action from the docket, plus DISMISS; the reassigned confirmation auto-clears on a bounded 3 s timer (a confirmation is not a standing notice). REASSIGN and DISMISS are focusable Buttons in the tab cycle; the requested card flashes a per-card `× ` denial (one bounded 0.45 s `FormTitleDanger` flash, token-guarded).
- **T34 capture fix:** none — the strip was already clean in every capture state.

### Tab grammar reuse (T32, registered here)

The Depot's BUY/SELL pair is the T26 zone-tab grammar applied to a second surface, verbatim: radio `ButtonGroup`, the `>> ` prefix + `Energized` variation on the active plate (never color alone), keyboard arrow hops between the pair + focused `ui_accept` to post, order-preserving content swap, tab state persisting across department switches within a session and resetting to BUY on boot. The SELL board lists only the resident's OWNED sellables, the disposal lines LEAD the board, and each row carries the honest quantity ladder SELL 1 / SELL 10% / SELL 25% / SELL 50% / SELL 100% / CUSTOM… — counts on the buttons, floor(stack × pct ÷ 100) in pure-int split div/mod math, 0 disables, exact proceeds in every tooltip, CUSTOM as the select-preview-commit field with out-of-range amounts refused through the refusal strip in the counter's voice.

### Compact card grammar — confirmed as built (T30), with the T34 cleanup

- **Confirmed:** the 2-column grid, card anatomy (30 px mark · short stencil name + designation digit · mono readout · 7 px micro gauge · filled badge at the row's right shoulder), the short-form register, role readouts (N CROWNS / N LINES / N/M POSTED), and the four energized cues (variation + `>> ` + amber readout + 1.05 swell) all match the T30 register. The captures show an even grid rhythm and aligned icon rows at 100 % and 200 % — no cleanup required on the wall itself.
- **T34 cleanup fix (capture-evidenced):** the O-1 slip's completion mark used to post as a full red `stamp_check` at every state — on a slip with one stamp it read as an eighth DONE box. It now speaks the slip's own fill grammar (a 20 px `MiniMark`): navy outline while the tutorial runs, the red stamp only at completion. Registered rule: **the fill is the state — a completion mark may never read as done while its fact is false.**
- **T34 cleanup fix (capture-evidenced):** the Personnel board summary now carries its own plural — ESTABLISHMENT · 1 POSTING · … (was "1 POSTINGS"). Registered rule: counts carry their own plural; the rest of the serial stays verbatim.

### Reveal/pulse motion rule — confirmed as built (T33), restated as the standing rule

One bounded settle pulse per reveal (0.5 s warm lift toward the amber signal tint + 2 % settle swell, back at rest), token-guarded so a re-reveal restarts one pulse and never stacks, no completion callback, no looping tween anywhere on the shell. The pulse is arrival emphasis only — the orient_arrow cue and the `>> ` prefix remain the state cues. T34 re-pins the rule suite-side: the acceptance walkthrough runs the machinery in 23 UI actions (budget 25), one pulse per landing.

## As-Built Addendum — Run 6: The Wall Reverts, The Cards Move Inside (2026-09-17, T35)

The user's correction on the run-5 build: "i want the items inside of the skills to be the condensed cards, not the overarching skills on the right, that was fine as a list. lets keep the progress bars attached though so you can easily see progress and what is active etc." The run-5 condensation direction was real, but it had landed on the wrong surface. This addendum supersedes the run-5 wall register (the "SkillCard wall" section above) where they conflict.

### The wall reverts (both ways, part 1)

- **The list is back:** the left wall is a single column of eight full-width department plates (2 px separation — the pre-T30 density, eight plates unscrolled inside the wall viewport at 1280×720/100 %). Full stencil names return (`SCAVENGING · 1`); the T30 short forms (SCAV/FORAGE/FORGE/COOK/PATROL) retire from the wall.
- **The T30 additions stay** (the user: "keep the progress bars attached so you can easily see progress and what is active"): every plate carries its mono readout (`CLR NN · into/step`, `N CROWNS`, `N LINES`, `N/M POSTED`); skill plates keep the 7 px `MicroGauge` XP line (riding the readout row's right end) and the filled deputy posting badge (at the name row's right shoulder). The energized state keeps every cue: `SkillCardEnergized` variation + `>> ` prefix + amber mono readout + 1.05 swell.
- **Traversal re-derived:** the tab chain and hotkeys 1–8 follow the list order; the pinned arrows walk the column (ui_down/ui_up; ui_left/ui_right stay put). The T30 short-form register leaves the DESIGN register with it; the manifest_board icon row remains in ASSETS.md (provenance-logged history of the T30 wall).

### The condensed cards move inside the dockets (both ways, part 2)

- **Where the run-5 direction lands:** the ITEMS INSIDE each skill docket — the gathering tier cards, the processing recipe cards, and the patrol fauna cards — are the condensed Melvor-style cards, in an adaptive `GridContainer`: **2-up where the width fits at 1280×720/100 %**, collapsing to one column where it does not (200 % font scale; the fit re-checks on resizes, font-scale changes and flushes — the grid's own overflowing width can never decide the fit, the docket scroll's viewport does).
- **Card anatomy:** the `SkillCard` / `SkillCardEnergized` Button pair (10/6 margins) at ~half the docket width; icon-led title row (22 px mark, autowrapped stencil name — the sanctioned wrapped shape); the honest rate/yield/stats/drops segment flows unchanged, every exact number still on the card; the clearance gate posts INLINE — the staircase glyph beside the compact red serial (`FormTitleDanger`, the registered red-on-bone pair), same verbatim grade + earning-path text, no DangerPlate block.
- **Progress attached:** the ACTIVE card carries a live XP meter (7 px `MicroGauge`, the clearance gauge's own engine math, bound flush by flush; hidden while idle — an idle card has no progress to post), and the fighting fauna card carries its HP instrument (`FaunaHP`, the live engagement fill; the battle board's gauges keep the exact digits). The docket's standalone clearance gauge vent is unchanged — one engine read drives both instruments.
- **Measured honesty:** the condensed stacks are measurably smaller than the pre-T35 stacked blocks at 1280×720/100 % (baselines measured on the parent commit: scavenging tiers 1606 px → 921 px; junksmithing recipes 3538 px → 1708 px; the patrol's sunny board 795 px → 634 px), with the honest-math strings pinned verbatim suite-side (`tests/test_cards.gd`).
- **Nothing else moves:** the docket topology (header plate → directive → live content → primary plate → footer serial), the BEGIN/END SHIFT primary, the T31 refusal strip (still child 0), the T33 reveal machinery (the reveal targets the same tier/recipe/fauna cards — now grid cells), and every contrast pair and the direction contract are untouched.
