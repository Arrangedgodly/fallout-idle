...
- **Card wall** (left ~third, T30): a condensed 2-column grid of eight compact department cards (run-5 amendment — "condensed cards more like Melvor's UI" in the signage idiom), 8 px grid separations, vertical scroll reserved for 200% font scale. Each card: the department's mark (30 px icon — skill marks for the five skills; role marks for the departments: the crowns at the Depot, the manifest board at the Manifest, the deputy badge at Personnel), the short stencil name with its designation digit, a mono readout, a 7 px XP micro gauge (`MicroGauge`: hairline recessed track, 3 px amber enamel fill — skills only), and the filled deputy badge when the department holds a posting (skills only). Readouts: skill cards post `CLR NN · into/step` (at max grade, `CLR NN · MAX`); the Depot posts its wallet; the Manifest its holdings count (`N LINES`); Personnel the posting count (`N/M POSTED`). **Short-form register (stencil caps, in-voice, trademark-swept):** SCAV (Scavenging) · FORAGE (Foraging) · FORGE (Junksmithing) · COOK (Cooking) · PATROL (Wasteland Patrol) · DEPOT (Requisition Depot) · MANIFEST · PERSONNEL — the digit after the name is the D-0n serial number and the hotkey (`SCAV · 1`); the full plate names live on the docket headers and tooltips. Reading order = department order (row-major); the tab chain and hotkeys 1-8 follow it; arrow keys walk the grid (within rows, then across).
...
- **Docket internals** (skill dockets): hidden-at-rest energized status plate → vent housing with the clearance gauge (a 12 px enamel line since T30 — the honest digits live in the mono readout below it) + mono readout → micro section header → cards VBox (8 px separation) → vent-housed stamped log.
